...
function ability.OnUpdate()
    ...
    for i = 1, NPCs.Count() do
        ...
        if npc and not Entity.IsAlive(npc) and ability.farmer[npc] then ability.farmer[npc] = nil end
    end
    ...
    for _, me in ipairs(mytable) do
        if me then
            ...
            if target and Menu.IsKeyDown(ability.spellkey) and not Input.IsInputCaptured() and Entity.IsAlive(target) and not Entity.IsDormant(target) then
                target = target
                ...
                if (not ability.channelling[me] or ability.channelling[me] < GameRules.GetGameTime()) and not NPC.IsChannellingAbility(me) and not NPC.HasModifier(me, "modifier_hoodwink_sharpshooter_windup") and not NPC.HasModifier(me, "modifier_spirit_breaker_charge_of_darkness") and not NPC.HasModifier(me, "modifier_monkey_king_bounce_leap") and not NPC.HasModifier(me, "modifier_snapfire_mortimer_kisses") and not NPC.HasModifier(me, "modifier_primal_beast_onslaught_windup") and not NPC.HasModifier(me, "modifier_primal_beast_onslaught_windup") and not NPC.HasModifier(me, "modifier_void_spirit_dissimilate_phase") and not NPC.IsStunned(me) then
                    if not ability.orbwalking[me] then
                        if Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() > NPC.GetAttackRange(me) or NPC.HasModifier(target, "modifier_ghost_state") or NPC.HasModifier(target, "modifier_item_ethereal_blade_ethereal") or NPC.HasModifier(me, "modifier_windrunner_focusfire") or NPC.HasModifier(me, "modifier_prevent_taunts") or NPC.HasModifier(me, "modifier_weaver_shukuchi") or not ability.safe_cast(me, target) then
                            ...
                            if not ability.attacking[me] or ability.attacking[me] < GameRules.GetGameTime() then
                                Player.AttackTarget(Players.GetLocal(), me, target)
                                ability.attacking[me] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + NPC.GetAttackTime(me)
                            end
                        end
                    else
                        if ability.orbwalking[me] + NPC.GetAttackTime(me) - (NPC.GetAttackTime(me) / 1.5) < GameRules.GetGameTime() and not NPC.HasModifier(me, "modifier_templar_assassin_meld") and not NPC.HasModifier(me, "modifier_faceless_void_chronosphere_speed") and not NPC.HasModifier(target, "modifier_bashed") then
                            if not ability.walking[me] or ability.walking[me] < GameRules.GetGameTime() then
                                NPC.MoveTo(me, Input.GetWorldCursorPos())
                                ability.walking[me] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + (math.random(6, 9) / 10)
                            end
                ...
                if not target and Menu.IsKeyDown(ability.spellkey) and not Input.IsInputCaptured() then
                    if not ability.walking[me] or ability.walking[me] < GameRules.GetGameTime() then
                        NPC.MoveTo(me, Input.GetWorldCursorPos())
                        ability.walking[me] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + (math.random(6, 9) / 10)
                    end
            ...
            if not Menu.IsKeyDown(ability.spellkey) and (NPC.HasModifier(me, "modifier_illusion") or NPC.IsCreep(me) and not NPC.HasModifier(me, "modifier_chen_holy_persuasion")) and NPC.GetActivity(me) == 1500 then
                local farm_pos = {}
                for x, box in ipairs(Entity.GetField(Entities.Get(1, "C_DOTAGamerulesProxy"), "m_pGameRules").GetField("m_NeutralSpawnBoxes")) do
                    table.insert(farm_pos, (box.GetField("neutralSpawnBoxes").GetField("m_vMinBounds") + box.GetField("neutralSpawnBoxes").GetField("m_vMaxBounds")):Scaled(0.5))
                end
                for x, unit in ipairs(Entity.GetUnitsInRadius(me, 99999, 0)) do
                    if unit and (NPC.IsTower(unit) or NPC.IsCreep(unit)) then
                        table.insert(farm_pos, Entity.GetOrigin(unit))
                    end
                end
                if #farm_pos > 0 then
                    table.sort(farm_pos, function (a, b) return Entity.GetAbsOrigin(me):Distance(b):Length2D() > Entity.GetAbsOrigin(me):Distance(a):Length2D() end)
                end
                    ...
                    if Ability.IsReady(handle.spell) and not NPC.IsStunned(me) and not NPC.IsSilenced(me) and not NPC.HasState(me, Enum.ModifierState.MODIFIER_STATE_INVISIBLE) then
                        for caster, date in pairs(ability.defender) do
                            if date.spell and NPC.GetActivity(date.unit) ~= 1500 and NPC.GetTimeToFace(caster, me) < 0.02 and Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(caster)):Length2D() < handle.distance then
                                if (Ability.GetBehavior(date.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) ~= 0 and not NPC.IsLinkensProtected(me) then
                                    ...
                                    elseif (Ability.GetName(handle.spell) == "antimage_counterspell" or Ability.GetName(handle.spell) == "weaver_shukuchi" or Ability.GetName(handle.spell) == "nyx_assassin_spiked_carapace" or Ability.GetName(handle.spell) == "life_stealer_rage") and date.time + date.castpoint - 0.15 - (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) < GameRules.GetGameTime() then
                                        ...
                                        ability.defender[caster] = nil
                                    elseif Ability.GetName(handle.spell) == "item_black_king_bar" and (Ability.GetDispellableType(date.spell) == 1 or Ability.GetDamageType(date.spell) == 2 and Ability.GetType(date.spell) == 1) and date.time + date.castpoint - 0.03 - (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) < GameRules.GetGameTime() then
                                        Ability.CastNoTarget(handle.spell)
                                        ability.defender[caster] = nil
                                ...
                                elseif (Ability.GetBehavior(date.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_POINT) ~= 0 or (Ability.GetBehavior(date.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET) ~= 0 then
                                    if (Ability.GetName(handle.spell) == "item_manta" and Ability.GetDamageType(date.spell) ~= 4 and ((Ability.GetBehavior(date.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_AOE) == 0 or (Ability.GetBehavior(date.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET) == 0) or Ability.GetName(handle.spell) == "nyx_assassin_spiked_carapace" or Ability.GetName(handle.spell) == "life_stealer_rage" or Ability.GetName(handle.spell) == "item_stormcrafter" or Ability.GetName(handle.spell) == "juggernaut_blade_fury" or Ability.GetName(handle.spell) == "puck_phase_shift" or Ability.GetName(handle.spell) == "item_black_king_bar" and Ability.GetDispellableType(date.spell) == 1) and date.time + date.castpoint - 0.15 < GameRules.GetGameTime() then
                                        Ability.CastNoTarget(handle.spell)
                                        ability.defender[caster] = nil
                                    elseif Ability.GetName(handle.spell) == "juggernaut_blade_fury" and date.time + date.castpoint - 0.03 - (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) < GameRules.GetGameTime() then
                                        Ability.CastNoTarget(handle.spell)
                        ...
                        if (not ability.sleeptime[Ability.GetName(handle.spell)] or GameRules.GetGameTime() > ability.sleeptime[Ability.GetName(handle.spell)]) then
                            ...
                            elseif Ability.GetName(handle.spell) == "morphling_morph_agi" then
                                if not NPC.HasModifier(me, "modifier_morphling_morph_str") then
                                    if (Entity.GetHealth(me) / Entity.GetMaxHealth(me) > 0.6 or NPC.HasModifier(me, "modifier_fountain_aura_buff") or NPC.HasModifier(me, "modifier_item_satanic_unholy") and Entity.GetHealth(me) / Entity.GetMaxHealth(me) > 0.3 or NPC.HasModifier(me, "modifier_flask_healing")) and Hero.GetStrength(me) > 1 then
                                        if not Ability.GetToggleState(handle.spell) then
                                            Ability.Toggle(handle.spell)
                                            ability.sleeptime[Ability.GetName(handle.spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                        end
                                    else
                                        if Ability.GetToggleState(handle.spell) then
                                            Ability.Toggle(handle.spell)
                                            ability.sleeptime[Ability.GetName(handle.spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                        end
                                    end
                                end
                            elseif Ability.GetName(handle.spell) == "morphling_morph_str" then
                    ...
                    if NPC.HasModifier(me, "modifier_morphling_replicate") then
                        if not string.find(Ability.GetName(handle.spell), "morphling_") and not Ability.IsPassive(handle.spell) and not Ability.IsItem(handle.spell) then
                            if Ability.GetCooldown(handle.spell) > 0 then
                                if Ability.GetCooldownLength(handle.spell) > 0 then
                                    ability.cooldown[Ability.GetName(handle.spell)] = Ability.GetCooldownLength(handle.spell)
                                end
                                ability.casttime[Ability.GetName(handle.spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1))
                            end
                            if ((Ability.GetBehavior(handle.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET) ~= 0 or (Ability.GetBehavior(handle.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) or (Ability.GetBehavior(handle.spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_POINT) ~= 0) and (Ability.GetTargetTeam(handle.spell) & Enum.TargetTeam.DOTA_UNIT_TARGET_TEAM_FRIENDLY) == 0 and Ability.SecondsSinceLastUse(handle.spell) > 3 and Ability.SecondsSinceLastUse(handle.spell) < 6 then
                                cast_replicate = true
                            end
                        end
                    else
                        if NPC.GetAbility(me, "special_bonus_unique_morphling_8") and Ability.GetLevel(NPC.GetAbility(me, "special_bonus_unique_morphling_8")) > 0 then duration = 36 else duration = 20 end
                        if ability5 and Ability.GetName(handle.spell) == "morphling_replicate" and Ability.SecondsSinceLastUse(handle.spell) > duration then
                            ability.cooldown, ability.casttime = {}, {}
                        end
                    ...
                    if target and not NPC.HasState(me, Enum.ModifierState.MODIFIER_STATE_INVISIBLE) and Entity.IsAbility(item) and (ability.sleeptime[Ability.GetName(item)] == 0 or GameRules.GetGameTime() > ability.sleeptime[Ability.GetName(item)]) and (Ability.GetName(item) == "item_trickster_cloak" or Ability.GetName(item) == "item_glimmer_cape" or (not ability.channelling[me] or ability.channelling[me] < GameRules.GetGameTime()) and not NPC.IsChannellingAbility(me)) and Ability.IsReady(item) then
                        for _, handle in pairs(ability.handle) do
                            if handle.spell and Entity.IsAbility(handle.spell) then
                                if Ability.GetName(item) == "item_blink" or Ability.GetName(item) == "item_overwhelming_blink" or Ability.GetName(item) == "item_swift_blink" or Ability.GetName(item) == "item_arcane_blink" then
                                    ...
                                    if Ability.IsReady(handle.spell) and ((Ability.GetDispellableType(handle.spell) == 1 and not Ability.IsItem(handle.spell) or Ability.GetName(handle.spell) == "templar_assassin_meld" or Ability.GetName(handle.spell) == "tinker_rearm" or NPC.HasModifier(me, "modifier_legion_commander_press_the_attack") or Ability.GetName(handle.spell) == "enigma_black_hole" or Ability.GetName(handle.spell) == "juggernaut_omni_slash" or Ability.GetName(handle.spell) == "legion_commander_duel" or Ability.GetName(handle.spell) == "axe_berserkers_call" or Ability.GetName(handle.spell) == "nevermore_requiem" or Ability.GetName(handle.spell) == "arc_warden_flux" or Ability.GetName(handle.spell) == "dark_willow_shadow_realm" or Ability.GetName(handle.spell) == "meepo_poof") and Ability.GetName(handle.spell) ~= "meepo_divided_we_stand" and Ability.GetName(handle.spell) ~= "mars_spear") then
                                        if Ability.GetName(handle.spell) == "techies_land_mines" or Ability.GetName(handle.spell) == "storm_spirit_ball_lightning" or NPC.HasModifier(me, "modifier_legion_commander_press_the_attack") or Ability.GetName(handle.spell) == "ancient_apparition_ice_blast" or Ability.GetName(handle.spell) == "meepo_poof" or Ability.GetName(handle.spell) == "death_prophet_exorcism" or Ability.GetName(handle.spell) == "dark_willow_shadow_realm" then handle.distance = 0 end
                                        if Ability.GetName(handle.spell) ~= "tinker_rearm" then
                                            if (handle.distance < 410 and Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < 1199 or handle.distance > 410 and Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < 1199 + handle.distance - 150) and (Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() > handle.distance or Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < handle.distance and Ability.GetName(handle.spell) ~= "item_blink") then
                                                ...
                                                if  Ability.GetName(item) == "item_arcane_blink" then
                                                    ability.sleeptime["item_wind_waker"], ability.sleeptime["item_cyclone"] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1.5
                                                end
                                            end
                                        else
                                ...
                                elseif (Ability.GetName(item) == "item_wind_waker" or Ability.GetName(item) == "item_cyclone") and #Heroes.InRadius(Entity.GetAbsOrigin(target), 900, Entity.GetTeamNum(target), 0) == 1 then
                                    ...
                                    if (Ability.GetName(handle.spell) == "nevermore_requiem" and NPC.GetModifier(me, "modifier_nevermore_necromastery") and Modifier.GetStackCount(NPC.GetModifier(me, "modifier_nevermore_necromastery")) > max_souls or Ability.GetName(handle.spell) == "jakiro_ice_path" or Ability.GetName(handle.spell) == "techies_land_mines" and not NPC.GetModifier(target, "modifier_techies_sticky_bomb_slow") or Ability.GetName(handle.spell) == "lina_light_strike_array") and Ability.IsReady(handle.spell) and Ability.IsReady(handle.spell) and Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < handle.distance + 150 then
                                        Ability.CastTarget(item, target)
                                        ability.sleeptime["jakiro_dual_breath"], ability.sleeptime["jakiro_macropyre"], ability.sleeptime["jakiro_ice_path"] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1.5
                                        ability.sleeptime["lina_dragon_slave"], ability.sleeptime["lina_fiery_soul"], ability.sleeptime["lina_light_strike_array"] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1.6
                                        ability.sleeptime["techies_suicide"], ability.sleeptime["techies_sticky_bomb"] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1.6
                                        ability.sleeptime["nevermore_shadowraze1"], ability.sleeptime["nevermore_shadowraze2"], ability.sleeptime["nevermore_shadowraze3"], ability.sleeptime["nevermore_requiem"] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 1
                                        ability.sleeptime[Ability.GetName(item)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                    end
                        ...
                        if Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < distance then
                            ...
                            elseif (Ability.GetBehavior(item) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET) ~= 0 and Ability.GetName(item) ~= "item_power_treads" then
                                ...
                                elseif Ability.GetName(item) == "item_satanic" or Ability.GetName(item) == "item_bloodstone" or Ability.GetName(item) == "item_magic_wand" or Ability.GetName(item) == "item_magic_stick" or Ability.GetName(item) == "item_faerie_fire" or Ability.GetName(item) == "item_essence_ring" then
                                ...
                                elseif Ability.GetName(item) == "item_bottle" then
                                    if (Bottle.GetRuneType(item) == 6 or Bottle.GetRuneType(item) == 2 or Bottle.GetRuneType(item) == 0) and Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < NPC.GetAttackRange(me) + 300 then
                                        Ability.CastNoTarget(item)
                                        ability.sleeptime[Ability.GetName(item)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                    end
            ...
            for a = 0, 15 do
                ...
                if not NPC.HasModifier(me, "modifier_illusion") and spell and not Ability.IsAttributes(spell) and (Ability.IsActivated(spell) or Ability.GetName(spell) == "monkey_king_primal_spring") and not Ability.IsHidden(spell) and Entity.IsAbility(spell) and Ability.GetName(spell) ~= "plus_high_five" and Ability.GetName(spell) ~= "techies_minefield_sign" then
                    ...
                    if ability5 and Ability.GetName(ability5) == "morphling_morph_replicate" then
                        for h = 1, Heroes.Count() do
                            local enemy = Heroes.Get(h)
                            if enemy and not Entity.IsSameTeam(me, enemy) then
                                for a = 0, 24 do
                                    local enemyspell = NPC.GetAbilityByIndex(enemy, a)
                                    if enemyspell and ability.casttime[Ability.GetName(enemyspell)] and ability.cooldown[Ability.GetName(enemyspell)] and GameRules.GetGameTime() - ability.casttime[Ability.GetName(enemyspell)] > ability.cooldown[Ability.GetName(enemyspell)] then
                                        cast_replicate = true
                                    end
                                end
                            end
                    ...
                    if Ability.GetName(spell) ~= "rubick_spell_steal" and Ability.IsReady(spell) and Ability.IsCastable(spell, NPC.GetMana(me)) and (not NPC.IsChannellingAbility(me) or Ability.GetName(spell) == "pudge_rot") then
                        if target and (ability.safe_cast(me, target) or (Ability.GetName(spell) == "nevermore_requiem" or Ability.GetName(spell) == "jakiro_ice_path" or Ability.GetName(spell) == "techies_sticky_bomb" or Ability.GetName(spell) == "techies_land_mines" or Ability.GetName(spell) == "lina_light_strike_array") and NPC.HasModifier(target, "modifier_eul_cyclone") or NPC.HasModifier(target, "modifier_wind_waker")) then
                                    ...
                                    if (Ability.GetBehavior(spell) & Enum.AbilityBehavior.DOTA_ABILITY_BEHAVIOR_NO_TARGET) ~= 0 then
                                        ...
                                        elseif Ability.GetName(spell) == "phantom_lancer_phantom_edge" then
                                            ...
                                            if Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() < max_distance[Ability.GetLevel(spell)] then
                                                Player.AttackTarget(Players.GetLocal(), me, target)
                                                ability.channelling[me], ability.sleeptime[Ability.GetName(spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() / 500, GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + Ability.GetCastPoint(spell)
                                            end
                                        ...
                                        elseif Ability.GetName(spell) == "morphling_morph_replicate" then
                                            if cast_replicate or NPC.HasModifier(me, "modifier_morphling_replicate") and NPC.HasModifier(me, "modifier_earthshaker_enchant_totem") then
                                                Ability.CastNoTarget(spell)
                                                ability.sleeptime[Ability.GetName(spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                            end
                                        ...
                                        elseif Ability.GetName(spell) == "templar_assassin_trap" then
                                            for _, unit in ipairs(Entity.GetUnitsInRadius(me, distance, 2)) do
                                                if unit and NPC.GetUnitName(unit) == "npc_dota_templar_assassin_psionic_trap" and Entity.GetAbsOrigin(unit):Distance(Entity.GetAbsOrigin(target)):Length2D() < 400 then
                                                    Ability.CastNoTarget(NPC.GetAbility(unit, "templar_assassin_self_trap"))
                                                    ability.sleeptime[Ability.GetName(spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                                end
                                            end
                                        ...
                                        elseif not Ability.IsStolen(spell) and 
                                            ...
                                            Ability.GetName(spell) == "ursa_enrage" or 
                                        ...
                                        elseif (Ability.GetTargetTeam(spell) & Enum.TargetTeam.DOTA_UNIT_TARGET_TEAM_FRIENDLY) == 0 then
                                            ...
                                            if not NPC.HasModifier(target, "modifier_antimage_counterspell") and not NPC.HasModifier(target, "modifier_item_lotus_orb_active") then
                                                ...
                                                elseif Ability.GetName(spell) == "enigma_malefice" then
                                                    for name, handle in pairs(ability.handle) do
                                                        if handle.spell and Entity.IsAbility(handle.spell) and (Ability.GetName(handle.spell) ~= "enigma_black_hole" or Ability.GetName(handle.spell) == "enigma_black_hole" and (Ability.GetCooldown(handle.spell) > 0 or Entity.GetAbsOrigin(me):Distance(Entity.GetAbsOrigin(target)):Length2D() > handle.distance)) then
                                                            Ability.CastTarget(spell, target)
                                                            ability.sleeptime[Ability.GetName(spell)] = GameRules.GetGameTime() + (NetChannel.GetAvgLatency(0) + NetChannel.GetAvgLatency(1)) + 0.5
                                                        end
                                                    end
